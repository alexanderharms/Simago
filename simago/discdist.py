...
def draw_disc_values(prob_obj, population, random_seed):
    ...
    if prob_obj.conditionals is None:
        population[prob_obj.property_name] = draw_from_disc_distribution(
            prob_obj.probabs, population.shape[0], random_seed
        )
    # Iterate over the various conditionals.
        ...
        for cond_index in prob_obj.conditionals.conditional_index.unique():
            ...
            probabs_df = prob_obj.probabs.query(
                "conditional_index == @cond_index"
            )
            # - Get the corr. segment of the population
            population_cond = get_conditional_population(
                prob_obj, population, cond_index
            )

            # - Draw the values
            population_cond[
                prob_obj.property_name
            ] = draw_from_disc_distribution(
                probabs_df, population_cond.shape[0], random_seed
            )
